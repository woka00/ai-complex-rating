...
Скачать шаблон можно прямо из UI (кнопка «Шаблон CSV»).

---

## 6 обязательных окон (по ТЗ)

1. **Проект** — список моделей, мета-информация, добавление/удаление
2. **Критерии** — редактирование весов, групп, включение/отключение
3. **Оценки** — таблица ввода, импорт CSV
4. **Расчёт K** — детальные результаты по каждой модели и группе
5. **Аналитика** — bar chart, stacked bar, radar, отчёт с рекомендацией
6. **Сравнение** — сравнительная таблица + анализ чувствительности
...
- **ML**: ultralytics (YOLO), torchvision, transformers (опционально)
